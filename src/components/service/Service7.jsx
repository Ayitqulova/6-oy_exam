export default function  Service7() {

    return(
        <section className="w-[80%] m-auto flex">
            <div className="flex flex-col gap-6 py-28" >
                <h4 className="font-semibold text-2xl opacity-[0.87] leading-[150%]  " >Information Technology</h4>
                <h1 className="font-bold text-[36px] leading-[150%] text-[#232536] w-[580px]  ">We want to get local identification in every corner of the world in this era of global citizenship. </h1>
                <p className=" w-[75%] font-normal text-[16px] text-[#232536] opacity-[0.6] leading-[150%]  ">Through True Rich Attended does no end it his mother since real had half every him case in packages enquire we up ecstatic unsatiable saw his giving Remain expense you position concluded. Through True Rich Attended does no end it his mother since real had half every.</p>
            </div>
            <img className=" mb-20 "  src="/images/Image (8).png" alt="" />

        </section>
    )

}